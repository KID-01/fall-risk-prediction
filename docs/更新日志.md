...
2. **T11.2/T11.4 原标"已完成"经核查降级**：T11.2 集成测试仅用合成关键点帧注入，未接入真实萤石流，非真实端到端；T11.4 Docker 仅有文件未经实际 `docker-compose up` 运行验证；且模型未训练导致端到端测试缺乏被测对象。均降级为部分完成。

3. **T11.3 性能基准测试无产出**：原任务整理标记"已完成"，但 `tests/` 仅 3 个功能测试文件（test_core/test_integration_monitor/test_smoke），全项目搜索 `benchmark`/`性能基准`/`perf_test` 无匹配。已降级为部分完成。

4. **任务计数错误**：原任务整理声明总数 40（25 完成 + 2 部分 + 13 未开始），但所列条目实际为 43（25+2+16）。已修正为 43（21 完成 + 6 部分 + 16 未开始）。

5. **对照依据文件名过时**：原引用 `log.md` 已被删除（见 git status），改为 `更新日志.md`。
...
- ✅ 已完成：21（核心算法管线、数据管线、模型代码、后端框架、前端看板、单元测试）
- 🔄 部分完成：6（T1.3 萤石 SDK、T3.1 姿态估计仅 MediaPipe、T5.6 评估脚本、T11.2 集成测试仅合成数据、T11.3 无性能基准脚本、T11.4 Docker 仅有文件未验证）
- ❌ 未开始：16（环境风险感知 T4.x、跌倒动作分类 T6.1、ONNX 导出 T5.7、管理后台 T9.2、边缘计算 T10.x、文档提交 T12.x 等）
- ⚠️ 关键阻塞链：模型未训练（T5.5 仅有脚本无权重）→ T8.2 推理无效 → T11.2 端到端测试无被测对象 → T11.3 性能测试无意义 → T11.4 部署无法验证 → T12.2 测试报告无法撰写；环境风险感知模块空白（影响 T5.2 多模态融合）；萤石未接入（影响真实流测试）
