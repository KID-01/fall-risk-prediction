...
对照项目源码逐项核查 `任务整理.md` 与本日志的历史记录，发现多处与实际代码不符，已修正。
...
6. **任务计数错误**：原任务整理声明总数 40（25 完成 + 2 部分 + 13 未开始），但所列条目实际为 43（25+2+16）。已修正为 43（15 完成 + 12 部分 + 16 未开始）。
...
| 文件 | 修正内容 |
|------|---------|
| `docs/任务整理.md` | 任务总数 40→43；T3.1 已完成→部分完成；T11.3 已完成→部分完成；更新对照依据为 `更新日志.md`；补全各任务源码核查证据；补充重要提醒第 5/6 条 |
| `docs/更新日志.md` | 2026-07-15 两段 YOLO-Pose 记录添加"未落地"警示标注；新增本核查记录段 |

### 当前真实进度

- ✅ 已完成：15（核心算法管线、数据管线、后端框架、前端看板、单元测试）
- 🔄 部分完成：12（T1.3 萤石 SDK、T3.1 姿态估计仅 MediaPipe、**T5.1~T5.5 模型代码就绪但训练集采集+算法迭代中**、T5.6 评估脚本、**T8.2 推理服务依赖未训练模型输出无效**、T11.2 集成测试仅合成数据、T11.3 无性能基准脚本、T11.4 Docker 仅有文件未验证）
- ❌ 未开始：16（环境风险感知 T4.x、跌倒动作分类 T6.1、ONNX 导出 T5.7、管理后台 T9.2、边缘计算 T10.x、文档提交 T12.x 等）
