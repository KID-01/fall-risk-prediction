...
from fall_mvp.risk_extensions.contract import risk_state  # noqa: E402
...
def _draw(img, vis_p, env_p, objs, mscore, mstate, escore, estate, overall, top_hazards, reasons, t,
    ...
    if extensions:
        ...
        points = trajectory.get("predicted_points", [])
        for p0, p1 in zip(points, points[1:]):
            cv2.line(img, (int(p0[0]), int(p0[1])), (int(p1[0]), int(p1[1])), (255, 255, 0), 2)
        ext_lines = [
            f"PERSON INDEX: {extensions.get('human_risk_index', 'UNKNOWN')}",
            f"ENV INDEX: {extensions.get('environment_risk_index', 'UNKNOWN')}",
            f"INTERACTION INDEX: {extensions.get('interaction_risk_index', 'UNKNOWN')}",
            f"ENGINEERING STATE: {extensions.get('overall_engineering_state_v0_3', 'UNKNOWN')}",
        ]
        x = max(8, img.shape[1] - 360)
        for i, ln in enumerate(ext_lines):
            cv2.putText(img, ln, (x, 22 + i * 22), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 2)
        cv2.putText(img, "Engineering indices, not probabilities", (x, 22 + len(ext_lines) * 22),
...
def main() -> int:
    ...
    ap.add_argument("--enable-risk-extensions", action="store_true",
                    help="启用 v0.3 可选 Lighting/Clutter/Trajectory/Interaction 工程扩展")
    args = ap.parse_args()

    cfg = default_config()
    v = cfg["visual"]; y = cfg["yolo"]; er = cfg["environment_risk"]; fs = cfg["fusion"]
    rx = cfg["risk_extensions"]
    rx["enabled"] = bool(args.enable_risk_extensions)
    ...
    trajectory_provider = CausalTrajectoryProvider(rx["trajectory"])
    if rx["enabled"]:
        summary["risk_extensions_summary"] = {
            "max_human_risk_index": 0.0,
            "max_environment_risk_index": 0.0,
            "max_interaction_risk_index": 0.0,
            "engineering_state_frames": {"LOW": 0, "MEDIUM": 0, "HIGH": 0, "UNKNOWN": 0},
            "wet_floor_source": "wet_floor_unavailable",
        }
    ...
    for idx, ts_ms, frame in _iter_frames(Path(args.input)):
        ...
        extensions = None
        if rx["enabled"]:
            lighting = compute_lighting_risk(frame, rx["lighting"])
            clutter = compute_clutter_risk(env_person, objs, rx["clutter"], frame.shape)
            trajectory = trajectory_provider.update(t, env_person or vis_primary)
            wet_floor = unavailable_wet_floor()
            interaction = compute_interaction_risk(trajectory, objs, wet_floor, rx["interaction"])
            human_index = None if mscore is None else round(mscore * 100.0, 2)
            available_env = [value for value in (
                None if escore is None else escore * 100.0,
                lighting.get("risk_index"),
                clutter.get("risk_index"),
            ) if value is not None]
            environment_index = round(max(available_env), 2) if available_env else None
            interaction_index = interaction.get("risk_index")
            if human_index is None:
                engineering_state = "UNKNOWN"
            else:
                combined_index = max(value for value in (human_index, environment_index, interaction_index)
                                     if value is not None)
                engineering_state = risk_state(combined_index)
            extensions = {
                ...
                "overall_engineering_state_v0_3": engineering_state,
                "lighting": lighting,
                "clutter": clutter,
        ...
        if extensions is not None:
            row["risk_extensions"] = extensions
        rows.append(row)
        summary["frames"] += 1
        if person_present:
        ...
        if extensions is not None:
            sx = summary["risk_extensions_summary"]
            sx["max_human_risk_index"] = max(sx["max_human_risk_index"], extensions["human_risk_index"] or 0.0)
            sx["max_environment_risk_index"] = max(
                sx["max_environment_risk_index"], extensions["environment_risk_index"] or 0.0)
            sx["max_interaction_risk_index"] = max(
                sx["max_interaction_risk_index"], extensions["interaction_risk_index"] or 0.0)
            sx["engineering_state_frames"][extensions["overall_engineering_state_v0_3"]] += 1
